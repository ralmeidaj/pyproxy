...
function confirmApprove() {
    if (! confirm(`Aprovar o tenant "${props.tenant.name}"?\n\nEle terá acesso completo à plataforma e poderá emitir boletos.`)) return
    approveForm.post(route('backoffice.tenants.approve', props.tenant.id))
}
...
<template>
    <BackofficeLayout>
        ...
        <div v-if="tenant.status === 'pending_approval'"
            class="mb-5 flex items-center justify-between gap-4 rounded-2xl bg-amber-50 border border-amber-200 px-6 py-4">
            <div>
                <p class="font-semibold text-amber-800">Aguardando aprovação</p>
                <p class="text-sm text-amber-600 mt-0.5">
                    Revise os dados do tenant e clique em Aprovar para liberar o acesso à plataforma.
                </p>
            </div>
            <button
                @click="confirmApprove"
                :disabled="approveForm.processing"
                class="flex-shrink-0 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors">
                {{ approveForm.processing ? 'Aprovando…' : '✓ Aprovar Tenant' }}
            </button>
        </div>
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                ...
                <dl class="grid grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 text-sm">
                    ...
                    <div v-if="tenant.notes" class="col-span-2 lg:col-span-4">
            ...
            <!-- Alterar Status — full width, layout horizontal -->
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <div class="flex items-start justify-between gap-6">
                    <div>
                        <h2 class="text-xs font-semibold text-[#2d7ab5] uppercase tracking-wider mb-1">Alterar Status</h2>
                        <p class="text-xs text-gray-400">Status atual:
                            <span :class="['font-medium px-1.5 py-0.5 rounded', statusColors[tenant.status]]">{{ tenant.status_label }}</span>
                        </p>
                    </div>
                    <div class="flex-1 max-w-lg">
                        <div v-if="!tenant.allowed_transitions?.length" class="text-xs text-gray-400">
                            Nenhuma transição de status disponível.
                        </div>
                        <div v-else>
                            <button v-if="!showStatusForm"
                                @click="showStatusForm = true"
                                class="text-sm border border-gray-200 hover:bg-gray-50 text-gray-700 px-4 py-2 rounded-xl transition-colors">
                                Alterar status
                            </button>
                            <form v-else @submit.prevent="submitStatus" class="flex flex-wrap gap-3 items-end">
                                ...
                                <div class="flex-1 min-w-48">
                                    ...
                                    <input v-model="statusForm.reason" type="text"
                                        class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#3a9fd8]/30" />
                                ...
                                <div class="flex gap-2">
                                    <button type="submit" :disabled="statusForm.processing"
                                        class="bg-[#3a9fd8] hover:bg-[#2889c8] disabled:opacity-60 text-white text-xs font-medium px-4 py-2 rounded-xl transition-colors">
                                        Confirmar
                                    </button>
                                    <button type="button" @click="showStatusForm = false"
                                        class="border border-gray-200 text-gray-600 text-xs px-4 py-2 rounded-xl hover:bg-gray-50 transition-colors">
                                        Cancelar
                                    </button>
                                </div>
                            </form>
            ...
            <!-- Configurações de Boleto — full width -->
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                    <div v-for="key in tenant.api_keys" :key="key.id"
                        ...
                        <div v-if="!key.revoked_at" class="flex items-center gap-3 ml-4 flex-shrink-0">
                            <a :href="route('backoffice.tenants.api-keys.edit', [tenant.id, key.id])"
                                class="text-xs text-[#3a9fd8] hover:text-[#2889c8] hover:underline">
                                Editar
                            </a>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                ...
                <div v-if="!tenant.status_history?.length" class="py-6 text-center text-xs text-gray-400">
